        ...
        with open(fname, 'r') as fr:
            ...
            with open(outfile, 'a') as fw:
                ...
                for line in lines:      
                    ...
                    substr_le = 'LE_ID="et1"'       
                    le8 = re.search(substr_le, line, re.IGNORECASE)
#KE2, KE4, TZ1
                    if le1 != None:
                        le_code = 'UG1'
                    if le2 != None:
...
if le_code == "ZA1":
    ...
    rdnet_in['DESCRIPTION'] = 'ADDISABABA_PLAN'
    rdnet_in['SHIPPINGCARRIER'] = 'INTERNAL'
...
for stop_location in stop_locations:
    ...
    # Create a condition to filter the rows you want to update
    condition = rdnet_in['STOPLOCATIONID'] == stop_location

    # Perform the update using `.loc` with vectorized operations
    rdnet_in.loc[condition, 'FIRSTDRIVER'] = str(drivers_trucks.loc[truck_index - 1, 'FIRSTDRIVER'])
    rdnet_in.loc[condition, 'FIRSTTRAILER'] = drivers_trucks.loc[truck_index - 1, 'FIRSTTRAILER']
    rdnet_in.loc[condition, 'VEHICLEID'] = drivers_trucks.loc[truck_index - 1, 'VEHICLEID']


    # for index, row in rdnet_in[rdnet_in['STOPLOCATIONID'] == stop_location].iterrows():
    #     rdnet_in.loc[index, ['FIRSTDRIVER']] = drivers_trucks.loc[truck_index - 1, ['FIRSTDRIVER']].astype(str)
...
elif le_code == "KE4" or le_code == "KE2" or le_code == "TZ1" or le_code == "ET1":
...
rdnet_in.to_csv('data/roadnet/inbound/rdnet_in.csv', index=False, lineterminator='\r\n')
